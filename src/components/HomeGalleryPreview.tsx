import Image from "next/image";
import Link from "next/link";

const PREVIEW_IMAGES = [
  "https://images.unsplash.com/photo-1585747860715-2ba37e788b70?w=600&q=80",
  "https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=600&q=80",
  "https://images.unsplash.com/photo-1621605815971-fbc98d665033?w=600&q=80",
];

export function HomeGalleryPreview() {
  return (
    <section
      className="bg-[#f5f5f5] py-16 sm:py-20 lg:py-24"
      aria-labelledby="gallery-preview-title"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2
          id="gallery-preview-title"
          className="font-heading mb-10 text-center text-3xl font-bold tracking-tight text-[#1a1a1a] sm:text-4xl"
        >
          Galerie rapide
        </h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-6">
          {PREVIEW_IMAGES.map((src, i) => (
            <div
              key={src}
              className="group relative aspect-[4/3] overflow-hidden rounded-lg"
            >
              <Image
                src={src}
                alt={`Royal Barber ${i + 1}`}
                fill
                sizes="(max-width: 640px) 100vw, 33vw"
                className="object-cover transition-transform duration-300 group-hover:scale-105"
              />
            </div>
          ))}
        </div>
        <div className="mt-10 text-center">
          <Link
            href="/galerie"
            className="inline-flex items-center rounded-md border-2 border-[#1a1a1a] px-6 py-3 text-base font-semibold text-[#1a1a1a] transition-colors hover:bg-[#1a1a1a] hover:text-[#f5f5f5] focus:outline-none focus:ring-2 focus:ring-[#d4af37] focus:ring-offset-2 sm:px-8 sm:py-4"
          >
            Voir plus
          </Link>
        </div>
      </div>
    </section>
  );
}
